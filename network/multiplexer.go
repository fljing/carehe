...
type IdentityHandler interface {
	GetIdentity([]byte) uint64
	SetIdentity([]byte, uint64)
}
...
func NewConnection(conn Socket, maxcount int, dh DataHandler, ih IdentityHandler, eh ErrorHandler) *Connection {
	...
	return &Connection{
		conn:       conn,
		applicants: make(map[uint64]chan []byte, count),
...
func (p *Connection) send() (err error) {
	defer func() {
		if err != nil {
			select {
			case <-p.chexit:
...
func (p *Connection) newIdentity() uint64 {
	return atomic.AddUint64(&p.identity, 1)
}
func (p *Connection) addApplicant(identity uint64, ch chan []byte) {
	p.mutex.Lock()
	defer p.mutex.Unlock()
	p.applicants[identity] = ch
}
func (p *Connection) popApplicant(identity uint64) (chan []byte, bool) {
	p.mutex.Lock()
	defer p.mutex.Unlock()
